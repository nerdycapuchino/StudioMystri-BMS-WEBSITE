...
export const getLeadsPipeline = () =>
    api.get('/leads/pipeline').then(r => r.data.data || r.data);

export const createLead = (data: Partial<Lead>) =>
    api.post('/leads', data).then(r => r.data.data || r.data);

export const updateLead = ({ id, data }: { id: string; data: Partial<Lead> }) =>
    api.patch(`/leads/${id}`, data).then(r => r.data.data || r.data);

export const updateLeadStage = ({ id, stage }: { id: string; stage: string }) =>
    api.patch(`/leads/${id}/stage`, { stage }).then(r => r.data.data || r.data);
