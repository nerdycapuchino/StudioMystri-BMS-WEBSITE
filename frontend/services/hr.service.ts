...
export const getEmployee = (id: string) =>
    api.get(`/hr/employees/${id}`).then(r => r.data.data || r.data);

export const createEmployee = (data: Record<string, unknown>) =>
    api.post('/hr/employees', data).then(r => r.data.data || r.data);

export const updateEmployee = ({ id, data }: { id: string; data: Record<string, unknown> }) =>
    api.patch(`/hr/employees/${id}`, data).then(r => r.data.data || r.data);

export const markAttendance = (data: Record<string, unknown>) =>
    api.post('/hr/attendance', data).then(r => r.data.data || r.data);
...
export const getHRSummary = () =>
    api.get('/hr/summary').then(r => r.data.data || r.data);

export const uploadEmployeeDoc = ({ id, file }: { id: string; file: File }) => {
    const fd = new FormData();
    fd.append('document', file);
    return api.post(`/hr/employees/${id}/documents`, fd).then(r => r.data.data || r.data);
};
