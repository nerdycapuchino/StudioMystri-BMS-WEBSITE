...
export const getProject = (id: string) =>
    api.get(`/projects/${id}`).then(r => r.data.data || r.data);

export const createProject = (data: Record<string, unknown>) =>
    api.post('/projects', data).then(r => r.data.data || r.data);

export const updateProject = ({ id, data }: { id: string; data: Record<string, unknown> }) =>
    api.patch(`/projects/${id}`, data).then(r => r.data.data || r.data);

export const updateProjectStage = ({ id, stage, progress }: { id: string; stage: string; progress?: number }) =>
    api.patch(`/projects/${id}/stage`, { stage, progress }).then(r => r.data.data || r.data);

export const addProjectPayment = ({ id, data }: { id: string; data: Record<string, unknown> }) =>
    api.post(`/projects/${id}/payments`, data).then(r => r.data.data || r.data);

export const getProjectPayments = (id: string) =>
    api.get(`/projects/${id}/payments`).then(r => r.data.data || r.data);

export const getProjectStats = () =>
    api.get('/projects/stats').then(r => r.data.data || r.data);
