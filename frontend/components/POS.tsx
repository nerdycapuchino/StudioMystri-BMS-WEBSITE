            ...
            <div className="flex-1 flex flex-col h-full border-r border-white/5 relative">
                <div className="p-4 border-b border-white/5 bg-surface-dark flex justify-between items-center gap-4">
                    <div className="relative flex-1">
                        ...
                        <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full bg-background-dark border border-white/10 rounded-full pl-10 pr-4 py-3 text-white focus:ring-1 focus:ring-primary outline-none" placeholder="Search Item / Scan Barcode..." autoFocus />
                    </div>
                    <div className="flex gap-2">
                        {categories.map(cat => (
                            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-2 rounded-full text-xs font-bold transition-all ${activeCategory === cat ? 'bg-white text-black' : 'bg-white/5 text-zinc-400 hover:text-white'}`}>{cat}</button>
                        ))}
                    </div>
                </div>
            ...
            <div className="w-[400px] xl:w-[450px] bg-surface-darker flex flex-col h-full border-l border-white/5 shadow-2xl relative z-10">
                <div className="p-4 bg-surface-dark border-b border-white/5 space-y-3">
                            ...
                            <div className="relative">
                                ...
                                {showCustomerSearch && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-surface-dark border border-white/10 rounded-xl shadow-2xl p-2 z-50">
                                        <input autoFocus className="w-full bg-black/20 p-2 rounded-lg text-sm text-white border border-white/5 mb-2 focus:border-primary outline-none" placeholder="Search Name / Phone..." value={custSearchTerm} onChange={e => setCustSearchTerm(e.target.value)} />
                    ...
                    <div className="flex bg-background-dark p-1 rounded-lg border border-white/5">
                        <button onClick={() => setOrderType('Local')} className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all ${orderType === 'Local' ? 'bg-white text-black shadow' : 'text-zinc-500 hover:text-white'}`}>Local (CGST+SGST)</button>
                        <button onClick={() => setOrderType('InterState')} className={`flex-1 py-1.5 text-[10px] font-bold uppercase rounded-md transition-all ${orderType === 'InterState' ? 'bg-amber-500 text-black shadow' : 'text-zinc-500 hover:text-white'}`}>Inter-State (IGST)</button>
                    </div>
                </div>
                    ...
                    {cart.length === 0 ? (
                        ...
                        cart.map((item, idx) => (
                            <div key={`${item.id}-${idx}`} className="bg-surface-dark p-3 rounded-xl border border-white/5 flex gap-3 group animate-in slide-in-from-right-2 duration-300">
                                <div className="size-12 bg-background-dark rounded-lg overflow-hidden shrink-0"><img src={(item as any).image || 'https://via.placeholder.com/100'} className="w-full h-full object-cover" /></div>
                ...
                <div className="p-4 bg-surface-dark border-t border-white/5 space-y-3">
                    ...
                    <div className="grid grid-cols-4 gap-2 pt-2">
                        <button onClick={() => setDiscount(prev => prev === 0 ? 5 : 0)}
                            className={`col-span-1 py-3 rounded-xl font-bold text-xs border ${discount > 0 ? 'bg-primary/10 text-primary border-primary/20' : 'bg-white/5 text-zinc-400 border-white/5'}`}>
                    ...
                    <div className="bg-surface-dark border border-white/10 rounded-[2.5rem] w-full max-w-4xl shadow-2xl p-6 md:p-10 space-y-6 max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-4">
                                <div className="space-y-1"><label className="text-[10px] font-bold uppercase text-zinc-500 ml-2">Product Name</label><input value={productForm.name} onChange={e => setProductForm({ ...productForm, name: e.target.value })} className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-primary" placeholder="Name" /></div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase text-zinc-500 ml-2">SKU Code</label><input value={productForm.sku} onChange={e => setProductForm({ ...productForm, sku: e.target.value })} className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white font-mono focus:outline-none focus:border-primary" placeholder="SKU" /></div>
                                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase text-zinc-500 ml-2">Barcode</label><div className="flex gap-2"><input value={productForm.barcode} onChange={e => setProductForm({ ...productForm, barcode: e.target.value })} className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white font-mono focus:outline-none focus:border-primary" placeholder="Scan or Gen" /><button onClick={generateBarcode} className="p-3 bg-white/5 rounded-xl hover:bg-white/10" title="Generate Barcode"><Barcode className="w-5 h-5" /></button></div></div>
                                </div>
                                <div className="grid grid-cols-3 gap-4">
                                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase text-zinc-500 ml-2">Selling Price</label><input type="number" value={productForm.price || ''} onChange={e => setProductForm({ ...productForm, price: Number(e.target.value) })} className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-primary" placeholder="0.00" /></div>
                                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase text-zinc-500 ml-2">Cost Price</label><input type="number" value={productForm.cost || ''} onChange={e => setProductForm({ ...productForm, cost: Number(e.target.value) })} className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-primary" placeholder="0.00" /></div>
                                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase text-zinc-500 ml-2">Initial Stock</label><input type="number" value={productForm.stock || ''} onChange={e => setProductForm({ ...productForm, stock: Number(e.target.value) })} className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-primary" placeholder="0" disabled={productForm.variants && productForm.variants.length > 0} /></div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase text-zinc-500 ml-2">Dimensions</label><input value={productForm.dimensions || ''} onChange={e => setProductForm({ ...productForm, dimensions: e.target.value })} className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-primary" placeholder="L x W x H" /></div>
                                    <div className="space-y-1"><label className="text-[10px] font-bold uppercase text-zinc-500 ml-2">Materials</label><input value={productForm.materials || ''} onChange={e => setProductForm({ ...productForm, materials: e.target.value })} className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-primary" placeholder="Wood, Metal..." /></div>
                                </div>
                                <div className="space-y-1"><label className="text-[10px] font-bold uppercase text-zinc-500 ml-2">Description</label><textarea value={productForm.description} onChange={e => setProductForm({ ...productForm, description: e.target.value })} className="w-full bg-background-dark border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-primary h-24" placeholder="Description..." /></div>
                            </div>
                            <div className="space-y-6">
                                <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                                    ...
                                    <div className="grid grid-cols-4 gap-2 mb-3">
                                        {productForm.media?.map((m, i) => (<div key={i} className="aspect-square rounded-lg overflow-hidden relative border border-white/10">{m.type === 'video' ? <video src={m.url} className="w-full h-full object-cover" /> : <img src={m.url} className="w-full h-full object-cover" />}</div>))}
                                        <label className="aspect-square rounded-lg border-2 border-dashed border-white/10 hover:border-primary/50 flex flex-col items-center justify-center cursor-pointer transition-colors"><Upload className="w-5 h-5 text-zinc-500 mb-1" /><span className="text-[9px] text-zinc-500 font-bold uppercase">Upload</span><input type="file" multiple accept="image/*,video/*" className="hidden" onChange={handleMediaUpload} /></label>
                                    </div>
                                </div>
                                <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                                    ...
                                    <div className="flex gap-2 mb-2 items-end">
                                        <div className="flex-1"><label className="text-[9px] uppercase font-bold text-zinc-500 ml-1">Name</label><input value={newVariant.name} onChange={e => setNewVariant({ ...newVariant, name: e.target.value })} placeholder="e.g. Red / Large" className="w-full bg-black/20 border border-white/10 rounded-lg p-2 text-xs text-white" /></div>
                                        <div className="w-20"><label className="text-[9px] uppercase font-bold text-zinc-500 ml-1">Price</label><input type="number" value={newVariant.price || ''} onChange={e => setNewVariant({ ...newVariant, price: Number(e.target.value) })} placeholder="0.00" className="w-full bg-black/20 border border-white/10 rounded-lg p-2 text-xs text-white" /></div>
                                        <div className="w-16"><label className="text-[9px] uppercase font-bold text-zinc-500 ml-1">Stock</label><input type="number" value={newVariant.stock || ''} onChange={e => setNewVariant({ ...newVariant, stock: Number(e.target.value) })} placeholder="0" className="w-full bg-black/20 border border-white/10 rounded-lg p-2 text-xs text-white" /></div>
                                    ...
                                    <div className="max-h-32 overflow-y-auto space-y-1 custom-scrollbar">
                                        {productForm.variants?.map((v, i) => (<div key={i} className="flex justify-between items-center text-xs bg-black/20 p-2 rounded"><span className="text-white font-bold">{v.name}</span><div className="flex gap-4"><span className="text-zinc-400">Stock: {v.stock}</span><span className="text-zinc-400">{formatCurrency(v.price)}</span></div></div>))}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-surface-dark border border-white/10 rounded-[2.5rem] w-full max-w-md p-8 shadow-2xl">
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-8">
                            <button onClick={() => handlePayment('Cash')} className="p-4 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-primary/50 rounded-2xl flex flex-col items-center gap-2 transition-all">
                                <Banknote className="w-8 h-8 text-green-400" /><span className="font-bold text-white text-sm">Cash</span>
                            </button>
                            <button onClick={() => handlePayment('Card')} className="p-4 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-primary/50 rounded-2xl flex flex-col items-center gap-2 transition-all relative group overflow-hidden">
                                <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 to-purple-500/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                                <div className="flex items-center gap-2 mb-1"><CreditCard className="w-8 h-8 text-blue-400" /><div className="h-6 w-px bg-white/10 mx-2" /><span className="text-[10px] font-bold text-zinc-400">RAZORPAY</span></div>
                                <span className="font-bold text-white text-sm relative z-10">Pay via Gateway</span>
                            </button>
                        </div>
