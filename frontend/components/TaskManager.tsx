                            ...
                            {(Array.isArray(tasks) ? tasks : []).filter((t: any) => t.status === status).map((t: any) => (
                                <div key={t.id} className="bg-white/5 p-4 rounded-xl border border-white/5 hover:border-primary/30 transition-all group relative">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <button onClick={() => deleteTask.mutate(t.id)} className="text-zinc-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="w-3 h-3" /></button>
                                    </div>
                                    <h4 className="font-bold text-white mb-1">{t.title}</h4>
                                    <p className="text-xs text-zinc-500 mb-3">{(Array.isArray(employees) ? employees : []).find((e: any) => e.id === t.assigneeId)?.name || 'Unassigned'}</p>
                                    <div className="flex justify-between items-center pt-2 border-t border-white/5">
                                        <div className="flex items-center gap-1 text-[10px] text-zinc-400"><Clock className="w-3 h-3" /> {t.dueDate ? new Date(t.dueDate).toLocaleDateString() : '-'}</div>
                                        {status !== 'Done' && (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
                    <div className="bg-surface-dark border border-white/10 rounded-[2rem] w-full max-w-md p-8 shadow-2xl space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <select value={form.priority} onChange={e => setForm({ ...form, priority: e.target.value })} className="bg-background-dark border border-white/10 rounded-xl p-3 text-white focus:border-primary focus:outline-none appearance-none">
