...
interface GlobalContextType {
  ...
  // RBAC Helper
  canEdit: (module: string, field: string) => boolean;
}
...
export const GlobalProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [exchangeRate, setExchangeRate] = useState(() => Number(localStorage.getItem('exchangeRate')) || 84);

  // Global Search State
  const [isSearchOpen, setIsSearchOpen] = useState(false);

  // RBAC Helper
  const canEdit = (module: string, field: string): boolean => {
    if (!currentUser) return false;
    if (currentUser.roleId === 'admin') return true; // Admin can always edit

    // Check specific permission matrix
    const perm = permissions?.find(p => p.role === currentUser.roleId && p.field === field);
    if (perm) return perm.access === 'read-write';

    return true; // Default to true if no specific restriction is set
  };

  useEffect(() => {
    localStorage.setItem('currency', currency);
    localStorage.setItem('exchangeRate', exchangeRate.toString()); // Persist rate
  ...
  }, [currency, exchangeRate, companySettings, salesToday, activities, projects, userRoles, currentUser, users, isShiftOpen, leads, notifications, invoices, employees, inventory, products, customers, systemLogs, teamMessages, teamChannels, shipments, policies, tasks, campaigns, integrations]);
