...
export const getMyTasks = () =>
    api.get('/tasks/my').then(r => ('data' in r.data ? r.data.data : r.data));

export const createTask = (data: Record<string, unknown>) =>
    api.post('/tasks', data).then(r => ('data' in r.data ? r.data.data : r.data));

export const updateTask = ({ id, data }: { id: string; data: Record<string, unknown> }) =>
    api.patch(`/tasks/${id}`, data).then(r => ('data' in r.data ? r.data.data : r.data));
