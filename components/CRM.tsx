...
export const CRM: React.FC = () => {
  const { leads, updateLead, updateLeadStatus, addLead, formatCurrency, deleteLead, currency } = useGlobal();
              ...
              {leads.filter(l => l.status === col).map(l => (
                ...
                >
                  <div className="flex justify-between mb-2">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); deleteLead(l.id); }} className="opacity-0 group-hover:opacity-100 text-zinc-600 hover:text-red-500 transition-opacity"><Trash2 className="w-3 h-3" /></button>
                  </div>
