...
import { Search, X, CreditCard, Banknote, Printer, Trash2, Box, Scan, Plus, User, MapPin, Calculator, FileText, ChevronDown, Image as ImageIcon, Upload, Layers, Barcode, Edit2, ShieldAlert, Lock } from 'lucide-react';

export const POS: React.FC = () => {
    const {
        addSale, addActivity, isShiftOpen, startShift, closeShift, products, addProduct, updateProduct, deductStock, formatCurrency, customers, addCustomer, addInvoice, companySettings, addInventoryItem, canEdit
    } = useGlobal();
            ...
            <div className="flex-1 flex flex-col h-full border-r border-white/5 relative">
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {/* ADD PRODUCT TILE (Restricted) */}
                        {canEdit('POS', 'manage_products') && (
            ...
            <div className="w-[400px] xl:w-[450px] bg-surface-darker flex flex-col h-full border-l border-white/5 shadow-2xl relative z-10">
                ...
                <div className="p-4 bg-surface-dark border-t border-white/5 space-y-3">
                    ...
                    <div className="grid grid-cols-4 gap-2 pt-2">
                        <button
                            disabled={!canEdit('POS', 'discount')}
                            onClick={() => setDiscount(prev => prev === 0 ? 5 : 0)}
                            className={`col-span-1 py-3 rounded-xl font-bold text-xs border ${discount > 0 ? 'bg-primary/10 text-primary border-primary/20' : 'bg-white/5 text-zinc-400 border-white/5'} disabled:opacity-50 disabled:cursor-not-allowed`}
                        >
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-8">
                            ...
                            <button onClick={() => handlePayment('Card')} className="p-4 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-primary/50 rounded-2xl flex flex-col items-center gap-2 transition-all relative group overflow-hidden">
                                ...
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="text-[10px] font-bold text-zinc-400">RAZORPAY</span>
                                </div>
